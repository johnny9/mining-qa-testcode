...
class PoolFallbackCases(MinerTestCase):
    """Pool transition correctness, independent of probe/reconnect policy."""
    settings_section = "pool_fallback_regression"
    protocol = "SV1"
    channel_type = None
    pool_type = WorkingPool
    required_capabilities = frozenset({caps.API, caps.MINING_STATE, caps.POOL_CONFIG, caps.STRATUM_V1})

    def setUp(self) -> None:
        super().setUp()
        self.settings = self.settings_for(self.settings_section)
        if self.settings.get("enabled", False) is not True:
            self.skipTest(f"enable tests.{self.settings_section}.enabled explicitly")
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.primary_pool = self.pool_type("primary", self.bind_host, self.difficulty, port=self.ports[0])
        self.fallback_pool = self.pool_type("fallback", self.bind_host, self.difficulty, port=self.ports[1])
        ...
        await self.pools.install(self.host, self.primary_pool.port,
                                 ...
                                 protocol=self.protocol, channel_type=self.channel_type,
                                 authority_keys=(self.primary_pool.authority_public_key_base58,
                                                 self.fallback_pool.authority_public_key_base58)
                                 if self.protocol == "SV2" else None)
    ...
    async def _phase(self, name: str, pool: WorkingPool, *, preference: int, fallback: int,
        ...
        def observe(sample: dict[str, Any]) -> None:
            ...
            append_jsonl(self.artifacts.path / "pool-phases.jsonl", {"phase": name, "protocol": self.protocol, "channel_type": self.channel_type, **sample})
    ...
    async def test_active_edits_and_manually_preferred_fallback(self) -> None:
        ...
        if self.protocol == 'SV2':
            self.pools.entries[primary]['stratumV2AuthorityPubkey'] = self.fallback_pool.authority_public_key_base58
